# blacknullify
/*
DONT USE ANY ` IN THE CODE UNTIL CONVERSION TOOL IS DONE
*/


// hooks componentComplete and makes it global under skip
(()=> {
  let objKeys = Object.keys;
  let hook;
  Object.keys = function () {
      if (arguments[0]?.componentComplete) {
          window.skip = arguments[0].componentComplete
          window.document.getElementById("skip-button").addEventListener("click", ()=> {
            setTimeout(() => { prompt('if you have not done 3 or more questions and waited a minute close this tab now and start again'); }, 900);
            setTimeout(() => { skip(100); }, 2000);
            setTimeout(() => { alert('reload iready in new tab'); }, 6000);
          })
      }
      return objKeys.apply(this, arguments);
  };
})()

function enableAllPerms() {
  let privileges = Object.values(document.getElementById('StudentDashboard-g38'))[1].children[2]._owner.stateNode.props.securityContext.privileges
  Object.keys(privileges).forEach(function(key, value) {
      return privileges[key] = true;
  })

  // refresh dash
  Object.values(document.getElementById("StudentDashboard-g38"))[1].children[0]._owner.stateNode.props.navigateToPage('features/dashboard/SELECT_SUBJECT')
}


//UI
const div = document.createElement("div");
div.id = "GUI";
div.className = "GUI";
div.style.position = "fixed";
div.style.top = "475px";
div.style.left = "381px";
div.style.transform = "translate(-50%, -50%)";
div.style.height = "260px";
div.style.width = "150px";
div.style.opacity = "0.85";
div.style.background = "black";
div.style.fontFamily = "sans-serif";
div.style.borderRadius = "10px";
div.style.display = "grid";
div.style.placeItems = "center";
div.style.color = "white";
div.style.fontSize = "larger";
div.style.zIndex = "99999";
div.style.cursor = "move";
// Second UI
const div2 = document.createElement("div");
div2.id = "GUI2";
div2.className = "GUI";
div2.style.position = "fixed";
div2.style.top = "475px";
div2.style.left = "381px";
div2.style.transform = "translate(-50%, -50%)";
div2.style.height = "160px";
div2.style.width = "150px";
div2.style.opacity = "0.85";
div2.style.background = "black";
div2.style.fontFamily = "sans-serif";
div2.style.borderRadius = "10px";
div2.style.display = "grid";
div2.style.placeItems = "center";
div2.style.color = "white";
div2.style.fontSize = "larger";
div2.style.zIndex = "99999";
div2.style.cursor = "move";
div2.style.display = "none";

const img = document.createElement("img");
img.id = "nullicon";
img.className = "center";
img.src = 'data:image/png;base64,';
div.appendChild(img);

const h1 = document.createElement("h1");
h1.className = "title";
h1.innerHTML = "<center>Nullify</center>";
div.appendChild(h1);

const skipButton = document.createElement("button");
skipButton.id = "skip-button";
skipButton.className = "button";
skipButton.innerHTML = "Skip Lesson";
div.appendChild(skipButton);

const allpermsButton = document.createElement("button");
allpermsButton.id = "all-perms-button";
allpermsButton.className = "button";
allpermsButton.innerHTML = "All Permissions";
div.appendChild(allpermsButton);

const discordButton = document.createElement("button");
discordButton.id = "discord";
discordButton.className = "button";
discordButton.innerHTML = "Discord";
div.appendChild(discordButton);

const miscButton = document.createElement("button");
miscButton.id = "misc-button";
miscButton.className = "button";
miscButton.innerHTML = "Misc";
div.appendChild(miscButton);

/*const backgroundImage = document.createElement("button");
      backgroundImage.id = "background-image";
      backgroundImage.className = "button";
      backgroundImage.innerHTML = "custom theme";
div.appendChild(backgroundImage);*/


document.body.appendChild(div2);

//Misc nullicon
const img2 = document.createElement("img");
img2.id = "nullicon";
img2.className = "center";
img2.src = 'data:image/png;base64,';
div2.appendChild(img2);
 

//Second UI buttons
const gamesButton = document.createElement("button");
gamesButton.id = "free-games";
gamesButton.className = "button";
gamesButton.innerHTML = "Free Games";
div2.appendChild(gamesButton);


 //color picker
 // Create a color picker input
const colorPicker = document.createElement("input");
colorPicker.type = "color";
colorPicker.id = "border-color-picker";
colorPicker.style.marginTop = "10px"; // Adjust the margin as needed
colorPicker.style.display = "none"; // Hide the color picker initially

const colorPickerButton = document.createElement("button");
colorPickerButton.id = "color-picker-button";
colorPickerButton.className = "button";
colorPickerButton.innerHTML = "menu theme";

div2.appendChild(colorPickerButton);

// Create a color picker input
colorPicker.addEventListener("input", (event) => {
  const selectedColor = event.target.value;
  div.style.background = `${selectedColor}`;
  // If you want to change the second UI (div2) border color as well:
  div2.style.background = `${selectedColor}`;
});


const style = document.createElement("style");
style.id = "bruh";
style.innerHTML = `

  .title {
    font-family: 'Nunito', sans-serif;
    font-size: 30px;
    color: white;
  }
    
  .button {
    font-family: 'Nunito', sans-serif;
    border: none;
    border-radius: 5px;
    color: white;
    text-align: center;
    font-size: 16px;
    cursor: pointer;
    transition-duration: 0.4s;
    width: 100%;
    text-align: center;
    color: white; 
    background-color: black;
  }
    
  .button:hover {
    color: black;
    background-color: grey;
  }
    
  .button:active {
    background-color: black;
  }

  .center {
    display: block;
    margin-left: auto;
    margin-right: auto;
  }
`;
div.appendChild(style);

document.body.appendChild(div);

// Make the GUI draggable
let isDragging = false;
let mouseOffsetX = 0;
let mouseOffsetY = 0;

div.addEventListener('mousedown', (event) => {
  isDragging = true;
  mouseOffsetX = event.clientX - div.offsetLeft;
  mouseOffsetY = event.clientY - div.offsetTop;
});

div.addEventListener('mouseup', () => {
  isDragging = false;
});

div.addEventListener('mousemove', (event) => {
  if (isDragging) {
    div.style.left = (event.clientX - mouseOffsetX) + 'px';
    div.style.top = (event.clientY - mouseOffsetY) + 'px';
  }
});

//make the second one draggable
let isDragging2 = false;
let mouseOffsetX2 = 0;
let mouseOffsetY2 = 0;

div2.addEventListener('mousedown', (event) => {
  isDragging2 = true;
  mouseOffsetX2 = event.clientX - div2.offsetLeft;
  mouseOffsetY2 = event.clientY - div2.offsetTop;
});

div2.addEventListener('mouseup', () => {
  isDragging2 = false;
});

div2.addEventListener('mousemove', (event) => {
  if (isDragging2) {
    div2.style.left = (event.clientX - mouseOffsetX2) + 'px';
    div2.style.top = (event.clientY - mouseOffsetY2) + 'px';
  }
});


window.document.getElementById("all-perms-button").addEventListener("click", ()=> {
  enableAllPerms()
})

window.document.getElementById("discord").addEventListener("click", ()=> {   
  window.open('https://discord.gg/eTpFvdmS6v', '_blank');
})

window.document.getElementById("free-games").addEventListener("click", ()=> {   
  window.open('https://docs.google.com/document/d/1w-uay3y5pwOm45sG9JqiSopKH9DO3pejABpUPBL-uiw/edit', '_blank');
})

window.document.getElementById("misc-button").addEventListener("click", () => {
  if (uiVisible) {
    div2.style.display = "grid";
  } else {
    div2.style.display = "none";
  }
  uiVisible = !uiVisible;
});

colorPickerButton.addEventListener("click", () => {
  colorPicker.click();
});




/*document.getElementById("background-image").addEventListener("click", function() {
  var src = prompt("Enter the image source:");
  this.setAttribute("src", src);
});*/

var nullify = document.createElement("div");
    (nullify.style.cssText =
      "position: fixed; bottom: 10px; width: 100%; color: white; font-size: 12px; text-align: center;"),
      (nullify.textContent = "SKID Menu Created by @varcreatefunction on Discord"),
      document.body.appendChild(nullify)

      let uiVisible = true;

      window.addEventListener("keydown", (event) => {
        if (event.code === "ShiftRight") {
          if (uiVisible) {
            div.style.display = "none";
          } else {
            div.style.display = "grid";
          }
          uiVisible = !uiVisible;
        }

      });




      //document.getElementById("background-image").setAttribute("src", prompt('data immage here'))
